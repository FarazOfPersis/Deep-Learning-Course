...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  id = torch.arange(x.shape[0] - 1, -1, -1)
  y = x[id].clone()
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def take_one_elem_per_col(x):
  ...
  id1 = torch.tensor([1, 0, 3])
  id2 = torch.tensor([0, 1, 2])
...
def normalize_columns(x):
  ...
  y = x.clone()
  y = y - mu
  sigma = (((y * y) / (x.shape[0] - 1)).sum(dim=0)).sqrt()
  y = y / sigma
  y = y.to(x.dtype)
  #############################################################################
  #                            END OF YOUR CODE                               #
